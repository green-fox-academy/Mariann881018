import javax.swing.*;
import java.awt.*;
import static javax.swing.JFrame.EXIT_ON_CLOSE;

public class RainbowBoxFunction {
  public static void mainDraw(Graphics graphics) {
    // Create a square drawing function that takes 3 parameters:
    // The square size, the fill color, graphics
    // and draws a square of that size and color to the center of the canvas.
    // Create a loop that fills the canvas with rainbow colored squares.
    // to the center then anywhere else???

    int size = 40;

    for (int i = 1; i <= 5; i++) {
      Color myNewColor = new Color(((int)(Math.random()*255)),((int)(Math.random()*255)),((int)(Math.random()*255)));
      graphics.setColor(myNewColor);
      colorCenterBox(size, myNewColor, graphics);
      }

    for (int i = 1; i <= 5; i++) {
      Color myNewColor = new Color(((int)(Math.random()*255)),((int)(Math.random()*255)),((int)(Math.random()*255)));
      graphics.setColor(myNewColor);
      randomBox(size, myNewColor, graphics);
    }

  }
  public static void colorCenterBox(int size, Color myNewColor, Graphics graphics){
    graphics.setColor(myNewColor);
    graphics.fillRect(((WIDTH/2)-(size/2)),(HEIGHT/2-(size/2)),size,size);
  }

  public static void randomBox(int size, Color myNewColor, Graphics graphics){
    graphics.setColor(myNewColor);
    graphics.fillRect(((int)(Math.random()*WIDTH)), ((int)(Math.random()*HEIGHT)), size,size);
  }



  // Don't touch the code below
  static int WIDTH = 320;
  static int HEIGHT = 320;
  public static void main(String[] args) {
    JFrame jFrame = new JFrame("Drawing");
    jFrame.setDefaultCloseOperation(EXIT_ON_CLOSE);
    RainbowBoxFunction.ImagePanel panel = new RainbowBoxFunction.ImagePanel();
    panel.setPreferredSize(new Dimension(WIDTH, HEIGHT));
    jFrame.add(panel);
    jFrame.setLocationRelativeTo(null);
    jFrame.setVisible(true);
    jFrame.pack();
  }

static class ImagePanel extends JPanel {
  @Override
  protected void paintComponent(Graphics graphics) {
    super.paintComponent(graphics);
    mainDraw(graphics);
  }
}
}